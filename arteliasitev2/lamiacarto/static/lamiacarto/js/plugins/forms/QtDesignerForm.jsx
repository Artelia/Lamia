...
class QtDesignerForm extends React.Component {
    ...
    componentDidMount() {
        console.log('qt componentDidMount')
    ...
    renderWidget = (widget, values, updateField, nametransform = (name) => name) => {
        ...
        if (widget.class === "QLabel") {
            ...
            widget.widget.forEach(function (widget, index) {
                ...
                if (index === 0) {
                    listdom.push(<input type="radio" key={3 * index} name="tabs" id={'tab' + index.toString()} style={{ display: 'None' }} defaultChecked />)
                } else {
                    listdom.push(<input type="radio" key={3 * index} name="tabs" id={'tab' + index.toString()} style={{ display: 'None' }} />)
                }

                listdom.push(<label key={3 * index + 1} for={'tab' + index.toString()} style={{ display: 'None' }}>{widget.name}</label>)
                listdom.push(
                    <div className="tab" key={3 * index + 2} style={{ display: 'block' }}>
            ...
            return (
                <div className="tabs" name={elname}>
                    {listdom}
                </div>
