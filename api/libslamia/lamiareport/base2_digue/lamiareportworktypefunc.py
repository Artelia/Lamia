...
from Lamia.iface.qgiswidget.tools.toolprepro.base2.lamiabase_graphique_tool import BaseGraphiqueTool
...
def profile(self):
    ...
    self.networkcore.exportCurrentGraph(point1=point1,
                                         point2=point2,
...
def profiltravers(self):
    ...
    lkressourceprofile = self.dbase.getValuesFromPk('Infralineaire',
                                                    'lid_ressource_4 ',
                                                    atlasfeat.id())
    ...
    if not self.dbase.utils.isAttributeNull(lkressourceprofile):
        ...
        sql = "SELECT typegraphique, pk_graphique FROM Graphique_qgis  WHERE id_ressource = " + str(lkressourceprofile)
        query = self.dbase.query(sql)
        result = [row for row in query]
        if len(result) > 0:
            ...
            exportfile = os.path.join(os.path.dirname(__file__), '..','..','..', 'config', 'tempgraph.png')
            graphtool.exportgraph(typegraphique,
                                    datas,
                                    exportfile,
                                        qrect.width(),
                                        qrect.height())
