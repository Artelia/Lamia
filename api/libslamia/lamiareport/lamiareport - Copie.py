...
class ReportCore:
    ...
    def runReport(
        ...
        self.impressionpdfworker = printPDFBaseWorker(
            reportcore=self,
...
class printPDFBaseWorker(object):
    ...
    def work(self):
        ...
        self.messageinstance.createProgressBar(
            inittext="Generation du pdf...", maxvalue=lenpks
        )
        ...
        """
        self.childstartheightpx = 0               #the height to communicate to childprint for starting
        self.currentstartheightpx = 0    #the height for starting print for this print
        self.pageheight = 0             #this print page height (without parent print)
        ...
        if 'childprint' in self.atlasconfData.keys() and len(self.atlasconfData['childprint'].keys()) > 0:
            ...
            if itemheightmm > 0.9 * compositionheightmm:
                self.pageheight += int(compositionheightmm / pageheightmm * self.painter.device().height())
            else:
                self.pageheight += int(itemheightmm / pageheightmm * self.painter.device().height())
            #self.currentstartheightpx = 0
            self.childstartheightpx +=  self.pageheight

        if self.parentprintPDFworker is not None and self.parentprintPDFworker.heightpx != 0:
            self.childstartheightpx += self.parentprintPDFworker.heightpx
            self.currentstartheightpx  = self.parentprintPDFworker.heightpx



        # if self.parentprintPDFworker is not None:
        #     self.childstartheightpx = 0
        #     self.currentstartheightpx  = self.parentprintPDFworker.heightpx
        ...
        #     parentheightmm = newComposition.pageItemBounds(newComposition.pageCollection().pageCount() - 1).height()
        #     pageheightmm = self.printer.paperRect(QPrinter.Millimeter).height()
        #     self.childstartheightpx = int(parentheightmm / pageheightmm * self.painter.device().height())
        #     self.currentstartheightpx = 0
        # else:
        #     self.childstartheightpx = 0
        #     self.currentstartheightpx = 0
        ...
        if self.parentprintPDFworker is not None:
            ...
            if linkcolumnname.split("_")[0] in ["lpk", "lid"]:
                ...
                self.messageinstance.showErrorMessage(
                    "Erreur sur la champs de liaison childprint"
                )
                return False
    ...
    def processImages(self, newComposition, atlas, atlasfeat):
        ...
        for imageitemname in self.atlasconfData["images"].keys():
            ...
            if os.path.isfile(self.atlasconfData["images"][imageitemname]):
                ...
                exec(
                    "imageresult = self.addonimagesmodule." + typeprocess + "(self)",
                    locals(),
                    globals(),
                )
    ...
    def getOrderedIdsForAtlas(self, coveragelayer):
        ...
        if (
            ...
            orderedids[0] = [
                feat[self.atlasconfData["atlaslayerid"]]
        ...
        if debug:
            self.logger.debug("orderedids %s", str(orderedids))
