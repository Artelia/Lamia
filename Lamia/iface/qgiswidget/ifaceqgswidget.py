...
class LamiaWindowWidget(QMainWindow,LamiaIFaceAbstractWidget):
    ...
    def loadToolsClasses(self):
        ...
        # ************************** LOAD prepro MODULES ********************************************
        if False:
            path = os.path.join(os.path.dirname(__file__), '.tools', 'toolprepro', self.dbase.worktype.lower())
            modules = glob.glob(path + "/*.py")
            __all__ = [os.path.basename(f)[:-3] for f in modules if os.path.isfile(f)]
            interfacefielduisup=[]
            ...
            for x in __all__:
                ...
                exec('import .tools.toolprepro.' + self.dbase.worktype.lower())
                moduletemp = importlib.import_module('.' + str(x), '.tools.toolprepro.' + self.dbase.worktype.lower() )
        ...
        if False:
            path = os.path.join(os.path.dirname(__file__), 'tools', 'toolpostpro',self.dbase.worktype)
            modules = glob.glob(path + "/*.py")
            __all__ = [os.path.basename(f)[:-3] for f in modules if os.path.isfile(f)]
            for x in __all__:
                exec('import .tools.toolpostpro.' + self.dbase.worktype)
                moduletemp = importlib.import_module('.' + str(x), '.tools.toolpostpro.' + self.dbase.worktype )
    ...
    def loadToolsWidgets(self, fullloading=False):
        ...
        for typewdg in typeswdg:
            ...
            for toolname in self.wdgclasses[typewdg].keys():
                ...
                if hasattr(self.wdgclasses[typewdg][toolname],'LOADFIRST') and not fullloading:
    ...
    #*************************************************************
    if False:
        def ____________________________________OLD(self):
            pass
    ...
    def tr(self, message):
        return QCoreApplication.translate('LamiaWindowWidget', message)
