        ...
        if isinstance(param1, QTreeWidgetItem) and (isinstance(param2, QTreeWidgetItem) or param2 is None):
            if param1 == self.qtreewidgetitem : #the openend qtreewidgetitem
                if (qgis.utils.iface is not None 
                        and self.DBASETABLENAME in self.mainifacewidget.qgiscanvas.layers.keys()
                        and 'layerqgis' in self.mainifacewidget.qgiscanvas.layers[self.DBASETABLENAME]):
                    qgis.utils.iface.setActiveLayer(self.mainifacewidget.qgiscanvas.layers[self.DBASETABLENAME]['layerqgis'])
        ...
        if self.mainifacewidget.interfacemode in [0,4]:
            ...
            if self.toolwidgetadvanced is None: #first loading
                ...
                if self.toolwidgetadvanced:
                    ...
                    self.formtoolwidgetconfdictadvanced = self.formtoolwidgetconfdictmain
