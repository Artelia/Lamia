...
class StatTool(AbstractLamiaTool):
    ...
    def __init__(self, dbase, dialog=None, linkedtreewidget=None, gpsutil=None, parentwidget=None, parent=None):
        ...
        self.abbv = {"AER": "Aérien", "CLA": "Clapet", "DIG": "Digue", "ENT": "Enterré", "NC": "Non-connu",
                     "REM": "Remarque", "VAN": "Vanne"}
    ...
    def getDataFrame(self):
        ...
        for date in self.dates:
            ...
            self.dictGraphs[date] = dict(Counter(self.cleanData(self.dbase.query(sql), 0)))
    ...
    def checkText(self):
        ...
        for element in queryList:
            if element.lower() in bannedQuery:
                QMessageBox.about(self, "Erreur".upper(),
                                  "Commande {operation} non-autorisée".format(operation=element.upper()))
    ...
    def getName(self):
        text, ok = QInputDialog.getText(self, "Créer une nouvelle requête", "Nom du fichier")
        if ok:
            return str(text + ".txt")
        return None
    ...
    def writeQuery(self):
        ...
        try:
            ...
            print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(), fct="writeQuery"))
            return False
    ...
    def drawGraph2(self):
        ...
        try:
            ...
            print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(), fct="drawGraph"))
        ...
        try:
            ...
            QMessageBox.about(self, "Erreur".upper(), "Aucune donnée trouvée")
            print("{type}: {exception} raised at {fct}".format(type=sys.exc_info()[0].upper(),
                                                               exception=e, fct="drawGraph > dict(self.counter)"))
            pass
        except:
            print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(), fct="drawGraph > dict(self.counter)"))
            pass
        ...
        try:
            ...
            # ----- Pie chart -----
                ...
                self.axtype.pie(sizes, labels=labels, autopct="%.0f%%", startangle=90, colors=colors)
    ...
    def setDefaultColor(self):
        ...
        for key in self.queryToSave.keys():
            if self.queryToSave[key] == "" and key != "dates":
                newColor = "#%02X%02X%02X" % (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
    ...
    def getData(self):
        ...
        if sender.objectName() == "pushButtonExecute":
            ...
            try:
                ...
                print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(),
                                                       fct="getData > updateQueryTableNow"))
        ...
        try:
            ...
            print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(), fct="getData"))
            return []
    ...
    def updateDetails(self, counter):
        ...
        if self.userwdgfield.tabResearch.currentIndex() == 0 or (
            ...
            try:
                ...
                print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(), fct="updateDetails"))
                pass
    ...
    def openColorDialog(self, key):
        ...
        if color.isValid():
            ...
            sender = self.sender()
            try:
                sender.setStyleSheet("background-color: {color}".format(color=color.name()))
            except Exception as e:
    ...
    def updateMaxOccurence(self, counter):
        ...
        try:
            ...
            print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(), fct="updateMaxOccurence"))
    ...
    def saveQuery(self):
        ...
        try:
            ...
            print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(), fct="saveQuery"))
    ...
    def modifySavedQuery(self):
        ...
        try:
            ...
            print("{type}: raised at {fct}".format(type=sys.exc_info()[0].upper(), fct="modifySavedQuery"))
            return False
    ...
    def confirmDetailled(self, title, text):
        ...
        :return: bool, True if accepted, False otherwise
        """
        confirm = QMessageBox.question(self, title, text, QMessageBox.Yes | QMessageBox.No)
