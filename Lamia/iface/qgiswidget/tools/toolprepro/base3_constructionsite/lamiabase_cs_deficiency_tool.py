...
from .lamiabase_cs_observation_tool import BaseConstructionsiteObservationTool as BaseObservationTool
...
from .lamiabase_cs_delivery_tool import BaseConstructionsiteDeliveryTool
...
class BaseConstructionsiteDeficiencyTool(BaseDeficiencyTool):
    ...
    tooltreewidgetSUBCAT = QtCore.QCoreApplication.translate('base3_cs','Non-compliance')
    ...
    def initMainToolWidget(self):
        ...
        if self.dbase.variante in [None, 'Lamia']:
            ...
            for i, (displayname, observationcategory,signing) in enumerate(self.nclist):
                ...
                temppropertieswdgOBSERVATION.TABLEFILTERFIELD = {'observationcategory': observationcategory }
            ...
            self.propertieswdgDelivery= BaseConstructionsiteDeliveryTool(**self.instancekwargs)
    ...
    def updateListSymbols(self):
        ...
        for i, (itemname,observationcategory, fieldsign ) in enumerate(self.nclist):
            if fieldsign and self.currentFeaturePK is not None :
                exec('obswdg = self.propertieswdgOBSERVATION{} '.format(observationcategory),locals(),globals() )
                iddes = self.dbase.getValuesFromPk('deficiency', 'id_deficiency', self.currentFeaturePK)
                ...
                if len(intervid)>0:
                    ...
                    sql = "SELECT {} FROM observation WHERE observationcategory = '{}' "\
                        " AND lid_deficiency = {} ".format(', '.join(intervid),
                                                            observationcategory,
                                                            iddes)
                    res = self.dbase.query(sql)
                    if res :
                        test = [re[0] for re in res if self.dbase.utils.isAttributeNull(re[0])  ]
                        if len(test) == 0 :
    ...
    def printWidget(self):
        ...
        currentid = self.dbase.getValuesFromPk('deficiency', 'id_deficiency',self.currentFeaturePK  )
        ...
        sql = "SELECT observationcategory FROM  observation_now WHERE observation_now.lid_deficiency = " + str(currentid)
        ...
        self.reporttool.runReport(destinationfile=pdffilename, 
...
class UserUI(QWidget):
    def __init__(self, parent=None):
        ...
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_cs_deficiency_tool_ui.ui')
        uic.loadUi(uipath, self)

class UserUI_Orange(QWidget):
    def __init__(self, parent=None):
        super(UserUI_Orange, self).__init__(parent=parent)
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_cs_deficiency_tool_orange_ui.ui')
        uic.loadUi(uipath, self)
