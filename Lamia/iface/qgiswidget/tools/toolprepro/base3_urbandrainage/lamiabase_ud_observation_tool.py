        ...
        if self.dbase.variante in [None, 'Lamia']:
            ...
            self.formtoolwidgetconfdictmain = {'observation': {'linkfield': 'id_observation',
                                                        'widgets': {
                                                            ...
                                                            'conditionglobal': [self.toolwidgetmain.comboBox_etatbache,
                ...
                if self.dbase.variante in [None, 'Lamia']:
                    if grpdes == 'NOD' and self.parentWidget.parentWidget is not None and self.parentWidget.parentWidget.currentFeaturePK is not None:
                        if self.parentWidget.parentWidget.DBASETABLENAME == 'node':
                            ...
                            typenoeud = self.dbase.getConstraintRawValueFromText('node', 'nodetype', currenttext)
...
class UserUI(QWidget):
    def __init__(self, parent=None):
        ...
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_ud_observation_tool_ui.ui')
        uic.loadUi(uipath, self)

class UserUI_2(QWidget):
    def __init__(self, parent=None):
        super(UserUI_2, self).__init__(parent=parent)
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_ud_observation_tool_ui_2018SNCF.ui')
        uic.loadUi(uipath, self)

class UserUI_3(QWidget):
    def __init__(self, parent=None):
        super(UserUI_3, self).__init__(parent=parent)
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_ud_observation_tool_ui_CD41.ui')
        uic.loadUi(uipath, self)