...
class BaseLeveeEdgeTool(BaseEdgeTool):
    ...
    def initMainToolWidget(self):
        ...
        self.formtoolwidgetconfdictmain = {'edge': {'linkfield': 'id_edge',
                                                    'widgets': {'edgetype': self.toolwidgetmain .comboBox_type,
                                            ...
                                            'descriptionsystem': {'linkfield': 'id_descriptionsystem',
                                                                'widgets': {'dateoperationalcreation' : self.toolwidgetmain .dateEdit_datecontruct,
    ...
    def initAdvancedToolWidget(self):
        ...
        self.formtoolwidgetconfdictadvanced = {'edge': {'linkfield': 'id_edge',
                                                        'widgets': {'edgetype': self.toolwidgetadvanced.comboBox_type,
                                                ...
                                                'object': {'linkfield': 'id_object',
                                                            'widgets': {'name': self.toolwidgetadvanced.lineEdit_nom,
    ...
    def postSelectFeatureAdvanced(self):
        ...
        idobjet = self.dbase.getValuesFromPk('edge_qgis',
                                             'id_object',
        ...
        #* photo
        lkressource = self.dbase.getValuesFromPk('edge_qgis', 'lid_resource_2', self.currentFeaturePK)
        if not self.dbase.utils.isAttributeNull(lkressource):
            sql = f"SELECT file FROM Ressource  WHERE id_ressource = {lkressource}"
            query = self.dbase.query(sql)
        ...
        #* profil travers
        lkressourceprofile = self.dbase.getValuesFromPk('edge_qgis', 'lid_resource_4', self.currentFeaturePK)

        if not self.dbase.utils.isAttributeNull(lkressourceprofile):
            sql = "SELECT file FROM Ressource  WHERE id_ressource = {lkressourceprofile}"
            query = self.dbase.query(sql)
            result = [row for row in query]
            ...
            sql = "SELECT pk_graph FROM graph_qgis  WHERE id_resource = " + str(lkressourceprofile)
            query = self.dbase.query(sql)
            result = [row for row in query]
            if len(result) > 0:
...
class UserUIField(QWidget):
    def __init__(self, parent=None):
        ...
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_levee_edge_tool_ui.ui')
        uic.loadUi(uipath, self)

class UserUIDesktop(QWidget):
    def __init__(self, parent=None):
        super(UserUIDesktop, self).__init__(parent=parent)
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_levee_edge_tooldesktop_ui.ui')
        uic.loadUi(uipath, self)