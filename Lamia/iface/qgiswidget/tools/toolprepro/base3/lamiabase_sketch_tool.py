...
class BaseSketchTool(AbstractLamiaFormTool):
    ...
    def initMainToolWidget(self):
        ...
        self.formtoolwidgetconfdictmain = {'media' : {'linkfield' : 'id_media',
                                            'widgets' : {}},
                                            'object' : {'linkfield' : 'id_object',
                                                        'widgets' : {}},
                                            'resource' : {'linkfield' : 'id_ressource',
                                                        'widgets' : {}}}
    ...
    def postSelectFeature(self):
        ...
        if self.currentFeaturePK is None:   #first time
            ...
            self.formutils.applyResultDict({'resource':{'datetimeresource' : datecreation}})
            self.editorwindow.reinitSize()
            self.editorwindow.clear()
            self.photowdg.clear()

        else:
            sql = "SELECT file FROM media_qgis  WHERE pk_media = " + str(self.currentFeaturePK)
    ...
    def postSaveFeature(self, savedfeaturepk=None):
        ...
        if self.currentFeaturePK is None:   #first creation
            ...
            sql = "UPDATE resource SET  file = '" + fileimage + "', datetimeresource = '" + datetimecreation + "'"
...
class UserUI(QWidget):
    def __init__(self, parent=None):
        ...
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_media_tool_ui.ui')
...
class ScribbleMainWindow(QMainWindow):
    def __init__(self, parentwdg=None, parent=None):
        ...
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_sketch_tool_drawingwidg_ui.ui')
        uic.loadUi(uipath, self)
