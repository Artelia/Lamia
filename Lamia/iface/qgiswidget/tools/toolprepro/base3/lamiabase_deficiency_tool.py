    ...
    def _checkSameGeomAsParentNode(self):
        if (self.currentFeaturePK is not None 
            ...
            if self.parentWidget.DBASETABLENAME in ['node']:
                parentgeom = self.dbase.getWktGeomFromPk('node', self.parentWidget.currentFeaturePK)
                ...
                parentgeom = self.dbase.getWktGeomFromPk('equipment', self.parentWidget.currentFeaturePK)
...
class UserUI(QWidget):
    def __init__(self, parent=None):
        ...
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_deficiency_tool_ui.ui')
