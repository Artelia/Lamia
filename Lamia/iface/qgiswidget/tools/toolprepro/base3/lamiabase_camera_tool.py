...
class BaseCameraTool(AbstractLamiaFormTool):
    ...
    def setDefaultPhoto(self):  
        ...
        idresource = self.dbase.getValuesFromPk('node',
                                                'id_resource',
                                                self.currentFeaturePK)

        sql = "UPDATE " + str(self.parentWidget.DBASETABLENAME) 
        sql +=  " SET  lid_resource_1 = " + str(idphoto) 
        sql += " WHERE id_" + + str(self.parentWidget.DBASETABLENAME)  + " = " + str(self.parentWidget.currentFeaturePK) + ";"
        ...
        if self.currentFeaturePK is None:   #first creation
            ...
            if self.parentWidget is not None and self.parentWidget.currentFeaturePK is not None:
                if self.parentWidget.DBASETABLENAME == 'node':
                    # get geom
                    noeudfetwkt = self.dbase.getValuesFromPk('node',
                                                            'ST_AsText(geom)',
...
class UserUI(QWidget):
    def __init__(self, parent=None):
        ...
        uipath = os.path.join(os.path.dirname(__file__), 'lamiabase_camera_tool_ui.ui')
