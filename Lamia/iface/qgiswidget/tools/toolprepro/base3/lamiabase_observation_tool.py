...
class BaseObservationTool(AbstractLamiaFormTool):
    ...
    CHOOSERTREEWDGSPEC = {'colshow': ['datetimeobservation'],
    ...
    def postSelectFeature(self):
        ...
        if self.currentFeaturePK is None:
            ...
            if self.parentWidget is not None and self.parentWidget.currentFeaturePK is not None:  #copy last obs text
                iddesordre = self.dbase.getValuesFromPk('deficiency',
                                                        'id_deficiency',
                                                        self.parentWidget.currentFeaturePK)
                sql = "SELECT pk_observation FROM observation_now WHERE lid_deficiency = {iddesordre}" \
                       " AND datetimeobservation = (SELECT MAX(datetimeobservation) FROM observation_now WHERE lid_deficiency = {iddesordre} )" .format(iddesordre=iddesordre)
    ...
    def postSaveFeature(self, savedfeaturepk=None):
        if self.currentFeaturePK is None:   #new feature
            ...
            if isinstance(datetimecreation, str):
                datetimecreation = datetime.datetime.strptime(str(datetimecreation), "%Y-%m-%d %H:%M:%S")
                datetimeobservation = datetime.datetime.strptime(str(datetimeobservation), "%Y-%m-%d %H:%M:%S")
            if datetimecreation > datetimeobservation:
                sql = "UPDATE object SET datetimecreation = '" + str(datetimeobservation) + "'"
                sql += " WHERE pk_object = " + str(pk_objet)
                self.dbase.query(sql)

            if self.parentWidget is not None and self.parentWidget.currentFeaturePK is not None:
                if self.parentWidget.DBASETABLENAME == 'deficiency':
                    pk_objet, datetimecreation = self.dbase.getValuesFromPk('deficiency_qgis',
