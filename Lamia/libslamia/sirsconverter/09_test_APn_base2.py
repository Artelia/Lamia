...
from Lamia.Lamia.libs.cloudant.client import Cloudant
from Lamia.Lamia.libs.cloudant.error import CloudantException
from Lamia.Lamia.libs.cloudant.result import Result, ResultByKey, QueryResult
from Lamia.Lamia.libs.cloudant.query import Query
...
def lst_val_class(db=None, clas=[], limit=1000000):
    ...
    for c in clas:
        print (c)
        print ('------------------------------------')
        query_collection = Query(my_db, selector={'@class': 'fr.sirs.core.model.{}'.format(c)})
...
def lst_key_ss_class(db=None, clas=[], limit=1000000):
    ...
    for c in clas:
        ...
        print (c)
        print ('------------------------------------')
        query_collection = Query(my_db, selector={'@class': 'fr.sirs.core.model.{}'.format(c[0])})
        for doc in query_collection(limit=limit, skip=0)['docs']:
            if c[1] in doc.keys():
                lst_ss_doc = doc[c[1]]
...
def analyse_param(doc, param, vars, dico_id):
    ...
    for p in param:
        val = None
        if "var" in p.keys(): #Si il s'agit d'une variable générée par le programme, on cherche sa valeur dans le dico
            if p['var'] in vars.keys():
                val = vars[p['var']]
        if "sirs" in p.keys(): #Si il s'agit d'une valeur contenue dans la BDD SIRS, on cherche sa valeur dans le document
            if p['sirs'] in doc.keys():
                if "dico_id" in p.keys(): #Si il s'agit d'un identifiant, on cherche sa correspondance dans le dico
                    ...
                    val = doc[p['sirs']]
        if "value" in p.keys(): #Si il s'agit d'une valeur fixée, on récupère celle ci dans les paramètres
            val = p['value']
        ...
        if val is not None: #Formatage des valeurs en fonction du type renseigné
            ...
            if p['type'] == "date":
                ...
                if val[0:12] == "LINESTRING (":
                    ...
                    val = 'GeomFromText("POINT ({})", {})'.format(lst_coord[(len(lst_coord) - 1) // 2], 2154)
...
def import_obs(obs, rs, param_obs, vars, dico_id):
    ...
    if param_obs['create_obj']:
        vars['$id_obj_cur_obs'] = lamia_insert(obs, rs, "Objet", param_obs['create_obj'], vars, dico_id)
        dico_id[vars['$id_sirs_cur_obs']]['id_obj'] = vars['$id_obj_cur_obs']
    if param_obs['insert_lamia']:
        vars['$id_elem_cur_obs'] = lamia_insert(obs, rs, param_obs['insert_lamia']['tab'],
                                                param_obs['insert_lamia']['fld'], vars, dico_id)
...
def import_pho(pho, rs, param_pho, vars, dico_id, rep_res_lamia):
    ...
    if param_pho['create_obj']:
        vars['$id_obj_cur_pho'] = lamia_insert(pho, rs, "Objet", param_pho['create_obj'], vars, dico_id)
        dico_id[vars['$id_sirs_cur_pho']]['id_obj'] = vars['$id_obj_cur_pho']
    if param_pho['create_res']:
        vars['$id_res_cur_pho'] = lamia_insert(pho, rs, "Ressource", param_pho['create_res'], vars, dico_id, rep_res_lamia)
        dico_id[vars['$id_sirs_cur_pho']]['id_res'] = vars['$id_res_cur_pho']
    if param_pho['create_obj_res']:
        lamia_insert(pho, rs, "Tcobjetressource", param_pho['create_obj_res'], vars, dico_id)
    if param_pho['insert_lamia']:
        vars['$id_elem_cur_pho'] = lamia_insert(pho, rs, param_pho['insert_lamia']['tab'],
                                                param_pho['insert_lamia']['fld'], vars, dico_id)
...
def import_sirs(only_valid=True):
    ...
    tabs = ['Objet', 'Descriptionsystem', 'Infralineaire', 'Equipement', 'Desordre', 'Observation', 'Ressource', 'Photo', 'Tcobjetressource']
    for t in tabs:
    ...
    for nm_class, param in config["import_table"].items():
        ...
        for doc in query_collection(limit=1000000, skip=0)['docs']:
            ...
            if param['create_obj']: # Créé un enregistrement dans la table Objet si nécessaire
                vars['$id_obj'] = lamia_insert(doc, rs, "Objet", param['create_obj'], vars, dico_id)
                dico_id[vars['$id_sirs']]['id_obj'] = vars['$id_obj']
            if param['create_des']: # Créé un enregistrement dans la table Descriptionsystem si nécessaire
                vars['$id_des'] = lamia_insert(doc, rs, "Descriptionsystem", param['create_des'], vars, dico_id)
                dico_id[vars['$id_sirs']]['id_des'] = vars['$id_des']
            if param['insert_lamia']: # Créé un enregistrement dans la table propre à la classe courante
                vars['$id_elem'] = lamia_insert(doc, rs, param['insert_lamia']['tab'], param['insert_lamia']['fld'], vars, dico_id)
                dico_id[vars['$id_sirs']]['id_elem'] = vars['$id_elem']
            if 'observations' in doc.keys(): # Traitement des observations liées au document courant
            ...
            if 'photos' in doc.keys(): # Traitement des photos liées au document courant
                lst_pho = doc['photos']
                for pho in lst_pho:
                    if param['import_pho']:
                        import_pho(pho, rs, param_pho, vars, dico_id, rep_res_lamia)
    ...
    # MAJ de la BDD Lamia
    for table in ["Objet", "Descriptionsystem", "Infralineaire", "Equipement", "Desordre", "Observation", "Ressource", "Photo"]:
...
def export_lamia(export_img=True):
    ...
    rs.execute("UPDATE objet SET importid = Null, importtable = Null WHERE importid = 1")
    bdd_sql.commit()

    # Création d'un dictionnaire de correspondance entre les IDs Lamia & les IDs SIRS (pour les tronçons)
    dico_infra = dict()
    rs.execute("SELECT pk_infralineaire, importtable FROM (objet INNER JOIN Descriptionsystem ON objet.pk_objet = Descriptionsystem.lpk_objet) "
    ...
    rs.execute("SELECT Objet.pk_objet as id_des, Infralineaire.pk_infralineaire as id_infra, MIN(ST_Distance(Desordre.geom, Infralineaire.geom)) as dist "
    ...
    rows = rs.fetchall()
    rs.execute("SELECT Objet.pk_objet as id_equip, Infralineaire.pk_infralineaire as id_infra, MIN(ST_Distance(Equipement.geom, Infralineaire.geom)) as dist "
    ...
    usr['_id'] = id_usr
    usr['designation'] = "ARTELIA {}".format(datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
    usr['valid'] = True
    ...
    for nm_class, param in class_to_import.items():
        if param['export']['obj']:
            ...
            for row in rows:
                ...
                if is_geom:
                    init_geom(rec)
                    rec['linearId'] = '{}'.format(dico_infra[dico_lnk_obj_infra[rec['#id_lamia_obj']]])
                    rec['foreignParentId'] = '{}'.format(dico_infra[dico_lnk_obj_infra[rec['#id_lamia_obj']]])
                    rec['geometry'], rec['positionDebut'], rec['positionFin'] = (create_sirs_geom(bdd_sql, nm_class,
                                                                                                  rec['#id_lamia'],
                                                                                                  dico_lnk_obj_infra[rec['#id_lamia_obj']]))
                l.append(rec)
                # Mise à jour de l'enregistrement dans Lamia, renseignement de l'ID SIRS créé
                rs.execute("UPDATE objet SET importid = 1, importtable = '{s}' WHERE pk_objet = {l}".format(s=rec[param['id']],
                                                                                                            l=rec['#id_lamia_obj']))
    ...
    for nm_class in ["PhotoTroncon", "PhotoEquip", "PhotoObs"]:
        ...
        if param['export']['obj']:
            ...
            for row in rows:
                ...
                if is_geom:
                    ...
                    if nm_class == 'PhotoTroncon':
                        ...
                        tronc_sirs = '{}'.format(dico_infra[dico_lnk_obj_infra[rec['#id_lamia_equip']]])
                        tronc_lamia = dico_lnk_obj_infra[rec['#id_lamia_equip']]
                    elif nm_class == "PhotoObs":
                        tronc_sirs = '{}'.format(dico_infra[dico_lnk_obj_infra[rec['#id_lamia_des']]])
            ...
            for data in l:
                ...
                if export_img:
                    img_lamia = os.path.join(os.path.dirname(nom_sql), rep_res_lamia, "Photo", data["chemin"])
...
def create_sirs_geom(bdd, nm_class, id_obj, id_infra):
    ...
    rs = bdd.cursor()
    rs.execute("SELECT ASTEXT(geom) FROM InfraLineaire WHERE pk_infralineaire = {}".format(id_infra))
    row = rs.fetchone()
    ...
    rs.execute("SELECT ASTEXT(geom), ST_GeometryType(geom) FROM {t} WHERE {f} = {i}".format(t=table, f=fld, i=id_obj))
    row = rs.fetchone()
    ...
    for pt in lst_pts:
        ...
        if (dst_min is None) or (dst_tmp < dst_min):
            (sqdist, projpoint, aftervertex, leftOf) = geom_infra.closestSegmentWithContext(pt)
...
def create_sirs_geom_by_sql(bdd, nm_class, id_obj, id_infra):
    ...
    rs.execute("SELECT " \
               ...
               "FROM " \
                    ...
                    "WHERE {t}.{f} = {o} AND Infralineaire.pk_infralineaire = {i})".format(t=table, f=fld, o=id_obj, i=id_infra))
    row = rs.fetchone()
    rs.close
...
def create_qry_export(nm_class, param):
    ...
    if nm_class == "Observation":
        ...
        sql_txt = "SELECT objet.pk_objet, Photo.pk_photo, tronc.pk_objet, tronc.importtable"
    elif nm_class == "PhotoEquip":
        fld_sirs = ['#id_lamia_obj', '#id_lamia', '#id_lamia_equip', '#id_equip']
        fld_type = ['int', 'int', 'int', 'str']
        sql_txt = "SELECT objet.pk_objet, Photo.pk_photo, equip.pk_objet, equip.importtable"
    elif nm_class == "PhotoObs":
    ...
    if param['insert_lamia']:
        for param_fld in param['insert_lamia']['fld']:
            if param_fld['lamia'] == 'geom':
                is_geom = True
            if recup_fld(param_fld):
                sql_txt = '{s}, {t}.{f}'.format(s=sql_txt, t=param['insert_lamia']['tab'], f=param_fld["lamia"])
                fld_sirs.append(param_fld["sirs"])
                fld_type.append(param_fld["type"])

    # Création du "FROM" de la requête en fonction de la classe de l'objet
    if nm_class == "Desordre":
        sql_txt = '{s} FROM objet INNER JOIN {t} ON objet.pk_objet = {t}.lpk_objet'.format(s=sql_txt, t=param['insert_lamia']['tab'])
    elif nm_class == "Observation":
        sql_txt = '{s} FROM ((Desordre INNER JOIN {t} ON Desordre.pk_desordre = {t}.lid_desordre) ' \
                  'INNER JOIN objet ON {t}.lpk_objet = objet.pk_objet) ' \
                  'INNER JOIN objet as obj_des ON Desordre.lpk_objet = obj_des.pk_objet'.format(s=sql_txt, t=param['insert_lamia']['tab'])
    elif nm_class == "PhotoTroncon":
        sql_txt = '{s} FROM(((((Photo INNER JOIN Ressource ON photo.lpk_ressource = Ressource.pk_ressource) ' \
                  ...
                  'INNER JOIN Infralineaire ON Descriptionsystem.pk_descriptionsystem = Infralineaire.lpk_descriptionsystem'.format(s=sql_txt)
    elif nm_class == "PhotoEquip":
        sql_txt = '{s} FROM(((((Photo INNER JOIN Ressource ON photo.lpk_ressource = Ressource.pk_ressource) ' \
                  'INNER JOIN Tcobjetressource ON ressource.id_ressource = Tcobjetressource.lid_ressource) ' \
                  ...
                  'INNER JOIN Equipement ON Descriptionsystem.pk_descriptionsystem = Equipement.lpk_descriptionsystem'.format(s=sql_txt)
    elif nm_class == "PhotoObs":
        sql_txt = '{s} FROM (((((((Photo INNER JOIN Ressource ON photo.lpk_ressource = Ressource.pk_ressource) ' \
                  'INNER JOIN Tcobjetressource ON ressource.id_ressource = Tcobjetressource.lid_ressource) ' \
                  ...
                  'INNER JOIN objet as des ON Desordre.lpk_objet = des.id_objet)'.format(s=sql_txt)
    else:
        sql_txt = '{s} FROM (Descriptionsystem INNER JOIN {t} ON Descriptionsystem.pk_descriptionsystem = {t}.lpk_descriptionsystem) ' \
                  'INNER JOIN objet ON descriptionsystem.lpk_objet = objet.pk_objet'.format(s=sql_txt,
                                                                                            t=param['insert_lamia']['tab'])
