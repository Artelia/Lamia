...
class printPDFBaseWorker(QtCore.QObject):
    ...
    def work(self):
        ...
        if (
            ...
            if self.dbase.base3version:
                strtoexec = f"Lamia.worktypeconf.{self.dbase.worktype.lower()}.lamiareport.lamiareportworktypefunc"
            else:
    ...
    def orderPksAlongPath(self, coveragelayer, zonegeopks=[]):
        ...
        if self.dbase.base3version:
            if len(zonegeopks) == 0:
                sql = "SELECT pk_geoarea FROM geoarea "
                query = self.dbase.query(sql)
            ...
            for zonegeopk in zonegeopks:
                ...
                sql = "SELECT ST_AsText(geom) FROM geoarea WHERE pk_geoarea = " + str(
                    zonegeopk
                )
                query = self.dbase.query(sql)
                zonegeogeom = [row[0] for row in query][0]
                sql = (
                    "SELECT edge_now.pk_edge FROM edge_now"
                    " WHERE ST_WITHIN(ST_StartPoint(edge_now.geom),ST_GeomFromText('{}',{}))".format(
            ...
            for zonegeopk in zonegeopks:
                ...
                sql = (
                    ...
                    " WHERE ST_WITHIN(ST_StartPoint(Infralineaire_now.geom),ST_GeomFromText('{}',{}))".format(
                        zonegeogeom, self.dbase.crsnumber
                    )
                )
                sql = self.dbase.sqlNow(sql)
    ...
    def setGeneralMapExtentByZonegeo(self, newComposition, zonegeopk):
        ...
        for mapname in self.atlasconfData["generalmap"].keys():
            if self.dbase.base3version:
                zonegeofet = self.qgiscanvas.layers["geoarea"]["layer"].getFeature(
                    zonegeopk
                )
            else:
