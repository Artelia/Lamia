...
class CostCore:
    ...
    def runCost(self, costfilepath, pkzonegeos=[]):
        ...
        if debug:
            self.logger.debug("***** bordereau : *****")
        if debug:
            self.logger.debug("%s", bordereau)

        restot = self.buildSQL(bordereau, pkzonegeos)
        if debug:
            self.logger.debug("***** restot : *****")
        if debug:
            self.logger.debug("%s", restot)

        ressql, resultfinal = self.priceCalculus(bordereau, restot)
        if debug:
            self.logger.debug("***** ressql : *****")
        if debug:
            self.logger.debug("%s", ressql)
        if debug:
            self.logger.debug("***** resultfinal : *****")
        if debug:
            self.logger.debug("%s", resultfinal)
        ...
        for i, line in enumerate(lines):
            ...
            if linecourread:
                ...
                lineprice = [
                    ...
                    for val in linesplitted[0 : len(bordereau["prix"])]
                ]
                ...
                if bordereau["condition"] is None:
                    ...
                    bordereau["condition"] = np.append(
                        bordereau["condition"], [linecondition], axis=0
                    )
                    bordereau["price"] = np.append(
                        bordereau["price"], [lineprice], axis=0
                    )

        if debug:
            self.logger.debug("bordereau")
        if debug:
            self.logger.debug("%s", str(bordereau))
    ...
    def buildSQL(self, bordereau, pkzonegeos):
        ...
        if self.dbase.base3version:
            if self.dbase.TYPE == "spatialite":
                sql = "SELECT CreateSpatialIndex('edge', 'geom')"
        ...
        if pkzonegeos:
            if self.dbase.base3version:
                wheresql = "geoarea_now.pk_geoarea IN ({})".format(
                    " ,".join([str(pk) for pk in pkzonegeos])
                )
            else:
                wheresql = "Zonegeo_now.pk_zonegeo IN ({})".format(
                    " ,".join([str(pk) for pk in pkzonegeos])
                )
        ...
        if debug:
            self.logger.debug("sql : %s", sqlfinal)
        ...
        if debug:
            self.logger.debug("res : %s", str(restot))

        if self.dbase.base3version:
            if self.dbase.TYPE == "spatialite":
                sql = "SELECT DisableSpatialIndex('edge', 'geom')"
                self.dbase.query(sql)
        elif self.dbase.__class__.__name__ == "SpatialiteDBaseParser":
        ...
        for i, res in enumerate(restot):
            ...
            for j, linecondition in enumerate(bordereau["condition"]):
                ...
                for k, tabelem in enumerate(linesimplified):
                    if ("<" in tabelem or ">" in tabelem) and ressimplified[
                        k
                    ] is not None:
            ...
            if indexconditionline is not None:
                if debug:
                    self.logger.debug(
                        "line %s, cond %s",
                        restrunc,
                        bordereau["condition"][indexconditionline],
                    )
                ...
                if debug:
                    self.logger.debug("line %s, cond %s", restrunc, "No condition")
                resultprices = [None] * len(bordereau["prix"])

            # finalprice
            finalprice = next(
                iter(item for item in resultprices[::-1] if item is not None), None
            )

            if debug:
                self.logger.debug("prices %s, finalprice %s", resultprices, finalprice)

            resultfinal.append(finalprice)

            if debug:
                self.logger.debug("resultfinal %s", str(resultfinal))
