...
class NetWorkCore():
    ...
    def getOrderedProjectedPks(self, pathpksreverse= None, layertoproject=None,constraint=''):
        """
        ...
        if geomprojection is not None:
            ...
            for fet in layertoproject.getFeatures():
                ...
                for point in [geom[0], geom[-1]]:
                    ...
                    nearestinfralinpk, dist = self.qgiscanvas.getNearestPk('Infralineaire',
                                                                       point,
    ...
    def getGraphData(self,  geomprojectionpks= None, datatype=None,graphtype='Profillong' ):  
        """
        ...
        if debug: logging.getLogger('Lamia').info('init geomprojectionpks : %s - datatype : %s - graphtype : %s',
                                                  str(geomprojectionpks),        str(datatype),  str(graphtype))
    ...
    def getInfralineairePkFromNxEdges(self,nxpathedges):
        ...
        for i in range(len(nxpathedges) - 1):
            ...
            # print('index1', index)
            if len(index) == 0:
                index = np.where(np.all(self.edgesreversenodeindex == singlepath, axis=1))[0]
                reverse = 1
            # print(ids)
            # print('index2', index)
    ...
    def getQgsGeomFromPks(self,pksreverse):    
        ...
        for pk, reversebool in pksreverse:
            ...
            #geom = feat.geometry()
            geomtxt = self.dbase.getValuesFromPk('Infralineaire',
                                                'ST_AsText(geom)',
                                                 pk)
    ...
    def computeNXGraph(self, listpks=[]):
        ...
        if listpks:
            if len(listpks) == 1 :
                sql += " WHERE Infralineaire_now.pk_infralineaire = " + str(listpks[0])
            else:
                sql += " WHERE Infralineaire_now.pk_infralineaire IN " + str(tuple(listpks))
        ...
        self.nxgraph, self.edgespks, self.nodegeom, self.edgesnodeindex, self.edgesreversenodeindex = nxgraph, pks, indexnoeuds, infralinfaces, reverseinfralinfaces
    ...
    def getSubGraphs(self):
        ...
        return [self.nxgraph.subgraph(c).copy() for c in networkx.connected_components(self.nxgraph)]
    ...
    def getExtremityNodeIndexFromLinearGraph(self, nxgraph):
        ...
        if len(edgenoeud)>0:    #graph is not a line
            ...
            return list(edgeextremite)     #logiquement cette list comporte deux elements : le noeud de depart et d 'arrivee
    ...
    def exportCurrentGraph(self,point1,point2, datatype,graphtype, w,h,exportfile):
        ...
        datas = self.getGraphData(geomprojectionpks=shortestpks, datatype=datatype,graphtype=graphtype)
        ...
        if datatype == 'topographie':
            for dataname in datas.keys():
                ...
                nom = self.dbase.getValuesFromPk('Topographie_qgis',
                                                   'Description',
                                                    topopk)
