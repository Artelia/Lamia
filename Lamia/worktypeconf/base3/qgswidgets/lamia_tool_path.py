    ...
    def selectPickedFeature(self, point):
        ...
        if False:
            ...
            xform = qgis.core.QgsCoordinateTransform(
                self.canvas.mapSettings().destinationCrs(), self.dbase.qgiscrs
            )
            success = qgis.core.QgsGeometry.fromPoint(pointfromcanvas).transform(xform)
            # print(pointfromcanvas)
        point2 = self.pointEmitter.toLayerCoordinates(
            self.dbase.dbasetables["Infralineaire"]["layerqgis"], point
        )
        # print(point2)

        if self.tempbutton == "start":
            self.userwdgfield.lineEdit_start.setText(
                str(point2.x()) + "," + str(point2.y())
            )
        elif self.tempbutton == "end":
            self.userwdgfield.lineEdit_end.setText(
                str(point2.x()) + "," + str(point2.y())
            )
        ...
        if self.indexnoeuds is not None:
            ...
            if False:
                ...
                for i in range(len(shortestpathedges) - 1):
                    ...
                    index = np.where(np.all(self.infralinfaces == singlepath, axis=1))[
                        0
                    ]
                    if len(index) == 0:
                        index = np.where(
...
class UserUI(QWidget):
    def __init__(self, parent=None):
        ...
        # self.setupUi(self)
        uipath = os.path.join(os.path.dirname(__file__), "lamia_tool_path_ui.ui")
