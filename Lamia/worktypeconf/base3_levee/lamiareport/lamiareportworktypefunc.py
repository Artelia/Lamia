...
def profile(self):
    ...
    exportfile = os.path.join(os.path.dirname(Lamia.__file__), 'config', 'tempgraph.png')
...
def profiltravers(self):
    ...
    #get graph pk
    resourceid = self.dbase.getValuesFromPk('edge',
                                            'lid_resource_4 ',
                                            atlasfeat.id())
    ...
    #make graph
    exportfile = os.path.join(os.path.dirname(Lamia.__file__), 'config', 'tempgraph.png')
    graphmaker.exportgraph(graphpk, exportfile, qrect.width(),
                                        qrect.height() )
    return exportfile
