...
from .lamia_form_sketch import BaseConstructionsiteSketchTool
from .lamia_form_camera import BaseConstructionsiteCameraTool
...
from Lamia.iface.qgiswidget.tools.form_subwidgets.subwidget_signature import (
    SignatureWidget,
)
...
class BaseConstructionsiteObservationTool(BaseObservationTool):
    ...
    def initMainToolWidget(self):
        ...
        self.formtoolwidgetconfdictmain["object"]["widgets"].update(
            {
                "comment": [
                    self.toolwidgetmain.textBrowser_ncbobs,
        ...
        if "observationcategory" in self.TABLEFILTERFIELD.keys():
            ...
            for i in range(self.toolwidgetmain.stackedWidget_2.count()):
                ...
                if (
                    self.toolwidgetmain.stackedWidget_2.widget(i).objectName()
                    == observationcategory
                ):
        ...
        # general
        self.propertieswdgPHOTOGRAPHIE = BaseConstructionsiteCameraTool(
            **self.instancekwargs
        )
        ...
        self.propertieswdgCROQUIS = BaseConstructionsiteSketchTool(
            **self.instancekwargs
        )
        self.propertieswdgCROQUIS.tooltreewidgetSUBCAT = "Plans"
        self.dbasechildwdgfield.append(self.propertieswdgCROQUIS)

        self.propertieswdgRapport = BaseConstructionsiteReportTool(
            **self.instancekwargs
        )
    ...
    def _initMainToolWidgetLamia(self):
        ...
        self.toolwidgetmain.stackedWidget_2.removeWidget(
            self.toolwidgetmain.stackedWidget_2.widget(0)
        )
        ...
        self.formtoolwidgetconfdictmain = {
            ...
            "object": {
                "linkfield": "object",
                "widgets": {"comment": self.ncawidget.textBrowser_ncathcommentaire,},
            },
        }
        ...
        if "observationcategory" in self.TABLEFILTERFIELD.keys():
            ...
            # mise dispo
                ...
                lidchooser = LidChooserWidget(
                    parentwdg=self,
                    parentlidfield="lid_delivery_2",
                    parentframe=self.toolwidgetmain.frame_entocc_marche,
                    searchdbase="delivery",
                    searchfieldtoshow=["name"],
                )
                self.lamiawidgets.append(lidchooser)
                signatureWidgetOccupante = SignatureWidget(
                    self,
                    "lid_actor_2",
                    "lid_resource_2",
                    "datetimesignature_2",
    ...
    def _initMainToolWidgetOrange(self):
        ...
        if "observationcategory" in self.TABLEFILTERFIELD.keys():
            ...
            if observationcategory == "NCA":
                ...
                self.tabWidget_page_formvisa.layout().addWidget(
                    self.tabWidget_multivisa
                )
                self.tabWidget_multivisa.addTab(QWidget(), "MOE")
                self.tabWidget_multivisa.addTab(QWidget(), "Entreprise")

                # visa nca MOE
                self.signatureWidgetMOE = SignatureWidget(
                    self,
                    "lid_actor_1",
                    "lid_resource_1",
                    "datetimesignature_1",
                ...
                # visa nca ETP
                self.signatureWidgetETP = SignatureWidget(
                    self,
                    "lid_actor_2",
                    "lid_resource_2",
                    "datetimesignature_2",
                ...
                # sous fiche
                self.sousficheWidget = BaseConstructionsiteSubObservationTool(
                    **self.instancekwargs
                )
    ...
    def changeGroupe(self, comboindex):
        parentcombotext = (
            self.parentWidget.toolwidgetmain.comboBox_groupedes.currentText()
        )
    ...
    def postSelectFeature(self):
        ...
        if self.currentFeaturePK is None:
            ...
            if (
                "observationcategory" in self.TABLEFILTERFIELD.keys()
                and self.currentFeaturePK is None
            ):
                observationcategory = self.TABLEFILTERFIELD["observationcategory"]
                ...
                if observationcategory == "NCB":
                    self.toolwidgetmain.dateEdit_datecible.setDateTime(
                        QtCore.QDateTime.fromString(valuedate, "yyyy-MM-dd")
                    )
                ...
                #     self.toolwidgetmain.dateEdit_etatverif.setDateTime(QtCore.QDateTime.fromString(valuedate, 'yyyy-MM-dd'))
                    ...
                    self.toolwidgetmain.dateTimeEdit_pva.setDateTime(
                        QtCore.QDateTime.fromString(
                            valuedatetime, "yyyy-MM-dd hh:mm:ss"
                        )
                    )

    def postSaveFeature(self, featurepk=None):
        # update tab symbol
        obstype = self.dbase.getValuesFromPk(
            self.DBASETABLENAME, "observationcategory", featurepk
        )
        for (
            lamiawidget
        ) in self.lamiawidgets:  # need to save before calling updateListSymbols
