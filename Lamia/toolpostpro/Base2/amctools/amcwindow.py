...
try:
    from qgis.PyQt.QtGui import (QWidget,QDialog,QMainWindow,QApplication,QTreeWidgetItem,
                                 QMenu,QSpinBox,QDoubleSpinBox,QHeaderView,QLineEdit,
                                 QTextBrowser,QTreeWidgetItemIterator, QComboBox, QLabel, UserRole, QInputDialog,
                                 QPushButton, QToolButton, QStatusBar)
except ImportError:
...
class AMCWindow(QDialog):
    ...
    def __init__(self, parent=None, dbase=None, jsonpath=None):
        ...
        if self.dbase.qgsiface is None:
            ...
            self.textBrowser_sqlfinal.setPlainText("FROM #lamia.noeud INNER JOIN #lamia.descriptionsystem ON descriptionsystem.pk_descriptionsystem = noeud.lpk_descriptionsystem")
        ...
        if False:
            ...
            self.filevirtuallayer = os.path.join(self.dbase.dbaseressourcesdirectory, 'config', 'amctools', jsonname+'.sqlite' )
        ...
        if True:
            ...
            jsonname = os.path.basename(os.path.splitext(self.jsonPath)[0])
            self.filevirtuallayer = os.path.join(self.dbase.dbaseressourcesdirectory, 'config', 'amctools',
                                                 jsonname + '.sqlite')

            # inform about status
            self.createdataframe.filename = self.filevirtuallayer
            self.createdataframe.projectcrs = self.dbase.qgiscrs
            self.toolButton_updatedb.setEnabled(False)
            self.getDBStatus()
    ...
    def visualisationPressed(self):
        ...
        provider.addAttributes([qgis.core.QgsField("amc_pk", QtCore.QVariant.Double),
        ...
        if debug:
            compt = 0
            for elem in self.dbase.dbasetables[maintable.split('_')[0]]['layerqgis'].getFeatures():
                compt += 1
                if compt >10:
                    break
                logging.getLogger("Lamia").debug('sqlsict %s %s', str(elem.attributes()[0]), str(elem.attributes()[-1]))
    ...
    def saveClicked(self):
        ...
        # self.dict['mainsql'] = self.lineEdit_sqlfinal.text()
    ...
    def loadFromJson(self, node, dct):
        ...
        for child in realChildren:
            ...
            self.loadWidgetProperties(qtreewidgetitm, dct[child]["select"], dct[child]["bareme"], dct[child]["type"],
                                      dct[child]["value"])
    ...
    def testDB(self):
        ...
        #insert it in sql
        sqlsplitted = self.dbase.splitSQLSelectFromWhereOrderby(sqltxt)
    ...
    def postVLayerProcessed(self, rawdata):
        ...
        if self.starttime is not None:
            self.appendMessage('end update  ' + str(round(time.time() - self.starttime, 2)) + ' seconds')
            self.starttime = None
            ...
            for sqlsplitspace in sqlssplitspace:
                if '#' in sqlsplitspace:
                    ...
                    if tabletype == '#lamia':
                        ...
                        vectorlayer = qgis.core.QgsVectorLayer(self.dbase.dbasetables[rawtablename]['layerqgis'].dataProvider().uri().uri(),
                                                               tablenamevlayer,
                                                               'spatialite')
                        ...
                        sql = "SELECT file FROM Rasters_now WHERE libelle = '" + str(tablename) + "'"
                        sql = self.dbase.updateQueryTableNow(sql)
                        res = self.dbase.query(sql)
                        if res and len(res) > 0:
                            shppath = res[0][0]
                            shpcompletepath = str(self.dbase.completePathOfFile(shppath))
                            if sys.version_info.major == 3:
                                tablenamevlayer = tablename
                                vectorlayer = qgis.core.QgsVectorLayer(shpcompletepath,
                                                                       tablenamevlayer,
                                                                       'ogr')
                    ...
                    if tabletype == '#lamia':
                        ...
                        if sys.version_info.major == 2:
                            if self.dbase.isTableSpatial(rawtablename):
                                vlayerlayer += str(
                                    QtCore.QUrl.toPercentEncoding("dbname='" + self.dbase.spatialitefile
                        ...
                        if res and len(res) > 0:
                            ...
                            if sys.version_info.major == 3:
                                ...
                                vlayerlayer += QtCore.QUrl.toPercentEncoding(shpcompletepath).data().decode('utf-8')
                                tablenamevlayer = tablename
    ...
    def calculus(self, node):
        ...
        res = []
        for line in dftonumpy:      # on s'en tirera à coup de boucle for .... il y a peut etre mieux mais ca marche
    ...
    def someFct(self, value, dct, typelist, defaultValue=0):
        ...
        if len(typelist) > 0 and typelist[0] in [int, float, np.int8, np.int16, np.int32, np.int64, np.float32, np.float64]:
            for key in dct.keys():
                if isinstance(dct[key], dict) and value[0] <= dct[key]["value"]:  # "value" in dct.keys() and
                    if self.legitChildren(dct[key]):
                        return self.someFct(value[1:], self.legitChildren2(dct[key]), typelist[1:], defaultValue)
                    else:
...
class CreateDataframe(QtCore.QObject):
    ...
    def __init__(self, scriptvl=None, filename=None):
        super(CreateDataframe, self).__init__()
        ...
        if not self.testsql :
            ...
            if os.path.isfile(self.filename):
                ...
                if updated :
                    ...
                    vlayer = qgis.core.QgsVectorLayer(sqlitefilename + self.scriptvl, "vlayer", "virtual")
