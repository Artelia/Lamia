...
class CroquisTool(AbstractInspectionDigueTool):
    ...
    def initTool(self):
        ...
        # userui
        self.userwdg = UserUI()
        self.linkuserwdg = {'Croquis' : {'linkfield' : 'id_croquis',
                                         'widgets' : {}},
        ...
        self.userwdg.pushButton_open.clicked.connect(self.openPhoto)
        self.userwdg.pushButton_edit.clicked.connect(self.editPhoto)
        self.editorwindow = ScribbleMainWindow()
        self.photowdg = Label()
        self.userwdg.frame_ph.layout().addWidget(self.photowdg)
    ...
    def postInitFeatureProperties(self, feat):
        ...
        if feat is not None :
            ...
            file = result[0]
            if os.path.isfile(self.completePathOfFile(file)):
                self.editorwindow.openImage(self.completePathOfFile(file))
                self.showImageinLabelWidget(self.photowdg, self.completePathOfFile(file))
        else:
    ...
    def openPhoto(self):
        if os.path.isfile(self.completePathOfFile(self.currentFeature['File'] )):
            filepath = self.completePathOfFile(self.currentFeature['File'])
    ...
    def createParentFeature(self):
        ...
        if not os.path.exists(os.path.dirname(self.completePathOfFile(fileimage))):
            os.makedirs(os.path.dirname(self.completePathOfFile(fileimage)))
        self.editorwindow.saveImage(self.completePathOfFile(fileimage))


        sql = "UPDATE Croquis SET id_objet = " + str(idobjet) + ",id_ressource = " + str(idres) + " WHERE id_croquis = " + str(idcroquis) + ";"
        query = self.dbase.query(sql)
        self.dbase.commit()
        sql = "UPDATE Ressource SET  file = '" + fileimage + "', Date = '" + datecreation + "'" + " WHERE id_ressource = " + str(idres) + ";"
        query = self.dbase.query(sql)
        self.dbase.commit()


        if self.parentWidget is not None and self.parentWidget.currentFeature is not None:
            if self.parentWidget.dbasetablename == 'PROFILTRAVERS':
...
class UserUI(QtGui.QWidget):
    def __init__(self, parent=None):
        super(UserUI, self).__init__(parent=parent)
        uipath = os.path.join(os.path.dirname(__file__), 'CroquisToolUser.ui')
        uic.loadUi(uipath, self)

class ScribbleMainWindow(QtGui.QMainWindow):
    def __init__(self, parent=None):
