...
class PhotosTool(AbstractInspectionDigueTool):
    ...
    def initTool(self):
        ...
        self.userwdg.pushButton_lastph.clicked.connect(self.lastPhoto)
        self.userwdg.pushButton_openph.clicked.connect(self.openPhoto)
        self.photowdg = Label()
        ...
        if self.parentWidget is not None and 'lk_photo' in self.dbase.dbasetables[self.parentWidget.dbasetablename]['fields'].keys():
            self.userwdg.pushButton_defaultphoto.clicked.connect(self.setDefaultPhoto)
        else:
            self.userwdg.pushButton_defaultphoto.setParent(None)
