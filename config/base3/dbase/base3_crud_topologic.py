...
from .base3_crud import LamiaORM as BaseLamiaORM
import qgis.core
...
class TopologicLamiaORM(BaseLamiaORM):
    ...
    class Node(BaseLamiaORM.AbstractTableOrm):
        def update(self, pk, valuesdict):
            ...
            self._moveLinkedTopologicalEdge(pk)

        def _moveLinkedTopologicalEdge(self, parentfeaturepk):

            if parentfeaturepk is not None:
                # nodeiddessys = self.dbase.getValuesFromPk(
                #     "node_qgis", ["id_descriptionsystem"], parentfeaturepk
                # )
                # nodegeom = self.formutils.getQgsGeomFromPk(parentfeaturepk).asPoint()
                # nodegeom = qgis.core.QgsGeometry(self.orm.)

                nodevals = self.orm.node.read(parentfeaturepk)
                nodeiddessys, nodegeom = (
                    nodevals["id_descriptionsystem"],
                    nodevals["geom"],
                )
                nodegeom = qgis.core.QgsGeometry().fromWkt(nodegeom).asPoint()

                # iterate on lid_descriptionsystem_1 and lid_descriptionsystem_2
                valuetoiterate = [1, 2]
                for indexnode in valuetoiterate:
                    sql = (
                        "SELECT pk_edge, id_edge FROM edge_now "
                        "WHERE lid_descriptionsystem_{} = {} ".format(
                            str(indexnode), str(nodeiddessys)
                        )
                    )
                    ...
                    for fetpk, fetid in result:
                        geomtext = self.dbase.getValuesFromPk(
                            "edge", "ST_AsText(geom)", fetpk
                        )
                        infrafetgeom = qgis.core.QgsGeometry.fromWkt(
                            geomtext
                        ).asPolyline()

                        if not self.dbase.utils.areNodesEquals(
                            ...
                            fetpk = self.orm._manageFeatureCreationOrUpdate(
                                "edge", fetpk
                            )
                            ...
                            sql = (
                                "UPDATE edge SET geom = ST_GeomFromText('{}',{}) "
                                " WHERE pk_edge = {}".format(
                                    newgeom.asWkt(), self.dbase.crsnumber, fetpk
                                )
                            )
        ...
        def _moveLaterals(self, pkinfralin, newgeom):
            ...
            fetiddessys = self.dbase.getValuesFromPk(
                "edge_qgis", "id_descriptionsystem", pkinfralin
            )
            ...
            for fetpk, fetid in result2:
                ...
                edgepk = self.orm._manageFeatureCreationOrUpdate("edge", fetpk)
                geomtext = self.dbase.getValuesFromPk("edge", "ST_AsText(geom)", edgepk)
                ...
                sql = (
                    "UPDATE edge SET geom = ST_GeomFromText('{}',{}) "
                    " WHERE pk_edge = {}".format(
                        newgeom2.asWkt(), self.dbase.crsnumber, edgepk
                    )
                )
                self.dbase.query(sql)
