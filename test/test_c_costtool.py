...
class DBaseTest(unittest.TestCase):
    ...
    def test_a_generateCost(self):
        ...
        slfile = SLFILE = os.path.join(
            os.path.dirname(__file__), "datas", "lamia_digue", "test01.sqlite"
        )
    ...
    def makeTests(self, dbtype, work, variante, dbase):
        ...
        costtool = CostCore(dbase, messageinstance=self.connector)

        conftotest = []

        for filename in glob.glob(os.path.join(costtool.confdatadirplugin, "*.csv")):
        ...
        for confname in conftotest:
            ...
            costtool.runCost(costfilepath=confname, pkzonegeos=[2])
