...
from Lamia.iface.qgiswidget.tools.toolpostpro.base2.Lamia_import_tool_flowchart import (
    FlowChartWidget,
)
...
class DBaseTest(unittest.TestCase):
    ...
    def test_a_generateImport(self):
        ...
        if SPATIALITE:
            slfile = os.path.join(
                self.testcdir, "sl_" + work + "_" + variante, "test01.sqlite"
            )
    ...
    def makeTests(self, dbtype, work, variante, dbase):
        importwdg = FlowChartWidget(
            dbase=dbase, messageinstance=self.connector, mainifacewidget=None
        )
        ...
        importwdg.initFromandToLayers(
            fromqgslayer=importlayer, tolayername="Infralineaire"
        )
