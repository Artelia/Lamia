...
class DBaseTest(unittest.TestCase):
    ...
    def makeTests(self,dbtype,  work, variante, dbase):
        ...
        exportershp = ExportShapefileCore(dbase,
                            messageinstance=self.connector)

        conftotest = []
        for filename in glob.glob(os.path.join(exportershp.confdataplugin, '*.txt' )):
        ...
        for confname in conftotest:
            logging.getLogger("Lamia_unittest").debug('********************** %s %s, %s - exporting conf : %s ...',dbtype, work, variante, basename)
            destfile = os.path.join(self.exportshpdir,dbtype + '_' + work + '_' + variante + '_' + confname  + '.shp')
            exportershp.runExport(destinationshapefile=destfile,
